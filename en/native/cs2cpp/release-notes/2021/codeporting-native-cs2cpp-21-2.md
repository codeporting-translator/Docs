...
1. The porter is now capable of the captured variables' lifetime prolongation. The delegate leaving original scope of its creation does no longer trigger a runtime error. The 'default_lambda_capture_mechanism' and 'avoid_lambda_holders_if_possible' porter options were added to control this behavior. For fine tuning, one may use the 'CppLambdaPassByReference', 'CppLambdaPassByValue', 'CppLambdaShouldCaptureByReference' and 'CppLambdaUseHolder' attributes.
1. The porter is now translating code from documentation comments so that the ported documentation has C++ look rather than C# one. This behavior can be controlled by the 'translate_code_from_comments' and 'translate_code_from_comments_system_dlls' porter options.
...
1. PrintTo method instantiation was fixed for some cases.
...
1. Some issues with includes and forward declarations in the ported code were fixed. This includes their instability, missing includes, ignoring CppForceInclude and CppForceForwardDeclaration attributes.
...
1. The exceptions thrown by the WebRequest and Directory classes were put in line with the .Net implementation.
...
1. More cmake target scripts were added into release package to make sure that Ninja build system works properly with MSVC libraries.
