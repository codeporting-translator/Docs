...
* FATAL ERROR - A fatal error that renders the continuation of the translation pointless and immediately terminates the application with error code 1. It is also displayed in red.

The strictness level can be configured via "strictness rules," which can be specified on the command line using the -sr flag. For example, `-sr T70 Fatal` will result in the translation process terminating immediately upon encountering unsupported code, without waiting for all files to be processed, and `-sr G* Info` will convert all C++ file generation issues into ordinary informational messages.
